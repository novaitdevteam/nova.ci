...
The switcher carries an **inline `sast-scan` job on `pull_request`** for the same thirteen
...
The switcher also carries an **inline `deps-scan` job on `pull_request`**, same thirteen

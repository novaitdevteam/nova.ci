...
Before emitting `runner_need=true`, the script takes a short-TTL **create lock** to close the check-then-act race between concurrent triggers (the winner's VM is not yet visible to the per-size count while it is being created): an atomic ref create of `refs/runner-locks/<size>` in `RUNNER_LOCK_REPO` (default `$GITHUB_REPOSITORY` — the caller's own repo), written with `RUNNER_LOCK_TOKEN` (default `$GH_TOKEN`), pointing at an annotated tag anchored at `$GITHUB_SHA` whose message is the acquisition epoch. A lock younger than `RUNNER_LOCK_TTL_SECONDS` (default 60s) sends the run to the wait queue with a `::notice::`; a stale lock (older, far-future, or unreadable epoch) is cleared and re-acquired; nobody releases the lock explicitly (TTL expiry hands the guard back to the per-size count). All lock-machinery failures fail **open** — create without the lock plus a `::warning::` — so a permissions problem can never block runner creation. Scoping the lock to the caller's own repo lets the built-in `GITHUB_TOKEN` satisfy the `contents: write` it needs with no cross-repo grant; this closes the dominant same-repo race but is **not** org-wide (a rarer cross-repo collision stays open, absorbed by the soft per-size cap plus the Hetzner watchdog). To restore org-wide scope, point `RUNNER_LOCK_REPO` at a shared repo and `RUNNER_LOCK_TOKEN` at a token with write there. Keep the lock fail-open.

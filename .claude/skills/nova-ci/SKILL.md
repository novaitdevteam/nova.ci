...
Before emitting `runner_need=true`, the script takes a short-TTL org-wide **create lock** to close the check-then-act race between concurrent triggers (the winner's VM is not yet visible to the per-size count while it is being created): an atomic ref create of `refs/runner-locks/<size>` in `RUNNER_LOCK_REPO` (default `<org>/nova.ci`), pointing at an annotated tag whose message is the acquisition epoch. A lock younger than `RUNNER_LOCK_TTL_SECONDS` (default 60s) sends the run to the wait queue with a `::notice::`; a stale lock (older, far-future, or unreadable epoch) is cleared and re-acquired; nobody releases the lock explicitly (TTL expiry hands the guard back to the per-size count). All lock-machinery failures fail **open** — create without the lock plus a `::warning::` — so a permissions problem can never block runner creation. `GH_TOKEN` needs `contents: write` on the lock repo for the lock to engage. Keep the lock fail-open and org-wide; repo-scoped GitHub `concurrency` groups are not a substitute (runner pools are org-wide).

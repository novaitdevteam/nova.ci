...
- Playwright E2E is **dispatch only**: `workflow_dispatch` in `novatalks.tests` → `ci-e2e-tests-manual.yaml` with `tests_ref` and `test_tags`, read in the switcher from `github.event.inputs`. No tag route — every run drops and restores the shared lab database, and the workflow's `concurrency: e2e-lab` group serializes runs. Future targets (an ephemeral stack from image tags) are new optional inputs with defaults, never a second workflow.

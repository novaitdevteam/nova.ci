...
<p align="center">
  <img src="assets/notifications.svg" width="100%" alt="a build notification carrying build status, lint status, unit test status, a Trivy line colour-coded by worst severity, a SAST line, a dependency line, a DAST line and an API scan line, plus a link to the reports on the build release; a DAST scan that never ran says so rather than reporting clean" />
</p>
...
- a **dependency line** — `📚 Deps (Trivy fs + OSV): 🟢 clean — Trivy and OSV both parsed a lockfile and found nothing`, `🟡 <n> finding(s) — Trivy <n>, OSV <n>`, `⚠️ no lockfile found — nothing was checked`, `❌ scan failed — <reason>`, or `⏭️ skipped (not a trunk build or scan* tag)`. The clean wording names *both* tools having parsed something on purpose, and `no lockfile found` is its own state rather than a zero: a repository nobody could scan and a repository with nothing to find produce the same count and must not produce the same line
...
> avoid — see [SAST and DAST](../security/sast-dast.md#a-scanner-that-could-not-run-is-not-a-clean-scan).
...
[Secret detection](../security/secret-detection.md#the-secret-scan-notify-job).

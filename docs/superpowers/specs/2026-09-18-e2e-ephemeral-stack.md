...
| D3 | **Images arrive as three form inputs, by explicit tag, with no "latest" lookup** | `engine_image`, `ui_image`, `botflow_image`. Same rule `ci-dast-pentest.yaml` already enforces: the registry's tag list is not date-ordered, so a "most recent" heuristic can silently test a stale image and report the run as current. Passing them per run is also what makes the stack useful beyond CI hygiene — a release candidate, a fix branch's build and the tag the stand runs are all one form field apart. Defaults are the tags the stand ran on 2026-09-18, so an operator who changes nothing gets today's known-good combination:<br>`ghcr.io/novaitdevteam/novatalks.core:2026_R3_NC2-2778_engine_f64f9736`<br>`ghcr.io/novaitdevteam/novatalks.ui:2026_R3_development_20fdf74f`<br>`ghcr.io/novaitdevteam/nova.botflow:2026_R3_master_f9126a11`<br>Pinned rather than floating on purpose: a moving default would mean a red run cannot be told from an image change. |

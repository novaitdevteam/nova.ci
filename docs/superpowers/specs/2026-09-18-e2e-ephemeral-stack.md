...
| D3 | **Images by explicit tag, no "latest" lookup** | Same rule `ci-dast-pentest.yaml` already enforces: the registry's tag list is not date-ordered, so "most recent" can silently scan a stale image. Three inputs — `engine_image`, `ui_image`, `botflow_image` — each defaulting to the tag the stand runs today. |
...
| D11 | **`medium` runner for `target: ephemeral`** | Measured on the stand: engine 1 CPU / 2 GB, postgres 500m / 2 GB, botflow 1 / 1.5 GB, ui 500m / 256 MB, redis 100m / 512 MB — about 3 CPU and 6.3 GB before a single browser starts. Four Chromium workers add roughly 2 GB. A `small` runner (4 vCPU / 8 GB) would be at its limit before the suite begins; `medium` (8 vCPU) is the first size that is not. |

...
fourth — whether the canonical BotFlow chatbot transfers a conversation to a team — was
closed by decision instead: it does not, so D7 copies the stand's own flows rather than
assembling from the canonical one.
...
| D7 | **The ephemeral stack mirrors the lab: its flows are the lab's own, copied at boot** | Decided 2026-09-21, replacing "assemble from the canonical flow". The target exists to be the same product as the stand, not a second dialect of it — and the difference is not theoretical: the canonical `BotAgent_Sys_ChatBot` does **not** transfer a conversation to a team the way the QA flow set does, which is what the reverted merge of 2026-09-17 proved on the stand itself. Building from the canonical flow would have meant closing that gap first, and every test that waits for an offer would have been the thing that reported it.<br><br>So the bring-up reads the live document from the stand's own Node-RED admin API (the same public URL and `E2E_BOTFLOW_ADMIN_*` credentials the suite already uses), then runs the existing slot reconcile against its own BotFlow for N workers. The stand stays the single source of truth for flow content, which is what was decided when flows left the repository, and the ephemeral stack stops being a place where flow logic can silently diverge.<br><br>The cost is honest and recorded: the ephemeral target is reproducible in its images and its configuration, but its flows are whatever the stand held at boot. So the run logs the node count and a hash of the document it copied — a red run can then be told from a flow change, which is the only property that copying really threatens. If the stand is ever unreachable at boot, that is a loud failure, not a fallback to something else: a suite that silently ran different chatbot logic is worse than one that did not run.<br><br>**Amended 2026-09-21**, after the stand went down and stopped every ephemeral run, local and CI: `E2E_FLOWS_FILE` names a document to use instead. That is not the fallback this decision rejects — it is never chosen automatically, naming it is a deliberate act, and the run prints a `::warning::` with the file and its age so a red run can still be told from a flow change. With the stand unreachable and no file named, the run still fails. Every successful copy is kept at `~/.cache/nova-e2e/flows-latest.json` so there is something to point at. |
| D8 | **`FILE_DRIVER=s3` against a bucket of its own, not `local`** | Production and the lab both run the S3 driver, and it is not a cosmetic difference: the attachment URL the app hands back is an object URL, not a path, and the suite uploads real files (`send_attachment` macros, the attachment dropzone). A stack on the local driver would be the one place those paths are exercised differently, which is exactly the class of bug a parity environment exists to catch rather than create. Credentials already exist in the tests repository (`E2E_R2_*`), so nothing new is provisioned for access.<br><br>**The existing bucket, with its own retention.** The engine takes a bucket name and nothing else — `AWS_S3_BUCKET`, no key prefix (checked against `file.config.ts`: ten `AWS_S3_*` keys, none of them a path), so ephemeral attachments land next to the stand's with nothing to tell them apart. That would be a growth problem if anything let it grow: the bucket carries a 30-day object retention, so the cleanup is a property of the storage rather than a teardown step that a cancelled run skips — which is the same reason the stand's prune runs before a suite rather than after. Nothing to provision, and the four values are already repository secrets. A separate bucket stays the answer only if separation itself ever matters; today it would buy a second thing to maintain and expire. `local` remains available as a switch for a run that must not touch the network at all. |
...
| D9a | ~~**Campaigns tests stay excluded on `lab`**~~ **Both targets run them**, since 2026-09-21 | Written when the lab had no NATS and so no campaigns module. It has one now: an account of its own (`lab-e2e`) on the `lab-evo` cluster, its own `campaign` stream, and the dialer enabled beside the engine — so the capability that made this a target property is no longer a difference between the targets.<br><br>The `@campaigns` tag stays, and its meaning inverts: it was the way to *skip* these tests (`chromium`'s `grepInvert` filtered it globally), and it is now the way to *select* them. That inversion is worth knowing, because tagging the spec while the exclusion still stood made it unreachable rather than runnable — the run reported `No tests found`, which is what a tag meaning "never" does when used to mean "only".<br><br>What the first runs on both targets showed: the campaigns sidebar link renders on each, so the module is genuinely on. The tests still fail, on UI interaction rather than on the module — at different points on each target, which is its own signal and belongs to QA rather than to CI. |
| D10 | **Everything is addressed on `localhost`, because the stack runs on the host network** | `ENV_URL=http://localhost:18080`, `BOTFLOW_URL=http://localhost:18080/redbot` — both through the one front proxy of D15, exactly as they are on the lab.<br><br>**Not 8080**: the E2E runner VM already has an nginx bound to `0.0.0.0:8080`, found on 2026-09-21 after it had answered the front proxy's health poll for three probe runs while our own nginx was failing to bind. What that nginx serves is still unknown. The origin is written in `stack.sh` and in the chart values that build every URL the product hands out, and the bring-up asserts the two agree — a mismatch would otherwise surface as a webhook posted to a port nobody serves, a long way from its cause.<br><br>The containers address each other as `127.0.0.1:<port>` rather than by container name: the whole stack runs with `--network host`, which is what lets `dast_bring_up_nats` be reused unchanged instead of parameterised for a bridge network, and a runner VM serves one job at a time so the fixed ports cannot collide. The engine's own webhook to BotFlow is therefore `http://127.0.0.1:1880/redbot/novatalks-botagent/1`. The lab's lesson still holds in the form that matters — an address that resolves to nothing produces silent webhook failures, which is what 451 of them on the lab came from when a fixture named a dead service. |
...
│   └── proxy    (nginx:alpine)   :18080 → the single origin on localhost
│                                          one origin, the lab's route table:
│                                          /redbot→botflow, /api /auth /ws /store
│                                          /widget /api-docs→engine,
│                                          /api/v1/dialer/→dialer, /→ui
│
│   ENV_URL=http://localhost:18080   BOTFLOW_URL=http://localhost:18080/redbot
...
| ~~Does the canonical flow transfer a conversation to a team~~ **Closed 2026-09-21 by decision, not by measurement** | — | It does not, as the reverted merge showed. D7 now copies the stand's own flows instead of assembling from the canonical one, so the question stops gating this work. It returns only if someone wants the canonical flow to become the source — which is a product decision about BotFlow, not a CI one. |
| ~~Boot time end to end~~ **Answered 2026-09-21: about 60s** | — | Probe run 35584025381 on `e2e-medium`, from postgres to a served origin: engine 38s (291 migrations and the full seed set against an empty database, with postgres durability off), dialer 10s, botflow 6s, ui 4s, proxy 2s. Well inside the 3-minute target, so viability is no longer the question — Step 4 measures whether it beats the lab's own queue for a 9-test smoke run, which is a different comparison. Images are pulled on first use, so a cold runner pays more than this; that too belongs to Step 4. |

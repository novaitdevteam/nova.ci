...
- A `unit-test` job runs sequentially after `linter` (`needs: [linter]`, `if: !cancelled()`) on both PR and non-PR events, so one build occupies one runner at a time instead of two. It is repo-aware: currently only `novatalks.core` runs `npm run test:unit`; all other standard build repos resolve to a no-op success (backward compatible). To enable a new repo, add a case in the "Resolve test plan" step. A no-op run reports `⏭️ n/a (no unit tests configured)` to the notifier instead of `✅`, so a repo that ran zero tests is not shown as passing; the gate result stays `success`.
- `build-image` has `needs: [linter, unit-test]`. Both lint and unit tests are advisory: the build continues if either fails. The condition is `!cancelled() && github.event_name != 'pull_request'`.
...
- The branch-push build route (`call-external-on-pull-request-merged`, "Call Builder On Merge PR") is commented out as legacy since 2026-08-12. `contains(github.event.head_commit.message, 'build')` matched the substring anywhere in the full commit message, so ordinary prose ("Specs build partial module graphs", "Verified: nest build") triggered image builds from feature branches and looked like PR builds. Keep the block commented, not deleted. If it is revived, gate it on an explicit marker (e.g. `[build]`) and/or a branch allowlist, never a bare `build` substring.
- The notifier job `needs: [build-image, linter, unit-test, trivy-scan]` and appends a Trivy line color-coded by worst severity (`🔴 CRITICAL found!` / `🟠 HIGH found` / `🟢 clean`, plus `❌ FAILED` under a fail mode or `⏭️ skipped`), with CRITICAL/HIGH counts and the report link, via a `Compose Trivy line` step that reads `trivy-scan` outputs. It also includes a `Unit Tests Status:` line (✅ / ❌ / `⏭️ n/a (no unit tests configured)`). The job summary shows a matching colored alert banner (CAUTION/WARNING/NOTE). Under the default `warn-only` mode the build stays green; the styling is the signal.
- The test workflow [`ci-build-ntk-on-push-tags-run-test.yaml`](.github/workflows/ci-build-ntk-on-push-tags-run-test.yaml) accepts `test_mode: unit | integration | both` (default `integration`). Tags containing `unit-test` → `unit`; `int-test` → `integration`; `full-test` → `both`. Integration tests use `npm run test:integration` with redis:8 services; unit tests use `npm run test:unit`. No `continue-on-error` on either job. `integration-tests` has `needs: [unit-tests]` with a `!cancelled()` condition, so `both` mode runs the suites sequentially on one runner (integration still runs if unit fails or is skipped). Integration sharding is intentionally not enabled by default (tests share DB state, run with `--runInBand`; each shard would need its own services).
...
- `novatalks.core` runner sizing in [`ci-build-create-runner.sh`](.github/workflows/ci-build-create-runner.sh) is differentiated by tag substring: `build` → `small` (cx33), `unit-test` → `medium` (cx43), `int-test`/`full-test` → `large` (cx53), else `small`. `unit-test` is matched before the generic `test` check. The matrix applies only to real tag pushes (`refs/tags/*`); branch and PR refs always resolve to `small`, so branch names containing `test` do not provision large VMs. Runner reuse only targets GitHub-registered runners whose backing Hetzner VM is `running` (ghost/deleting registrations are skipped). Each size class has its own max-2 concurrency cap, counted from Hetzner server state (`starting`/`initializing`/`running` of the size's `server_type`), not GitHub runner registrations; `medium` and `large` pools are independent so unit and integration runs do not contend. All other repositories always resolve to `small`, regardless of tag. All size pools additionally share a global `MAX_TOTAL_RUNNERS` cap (env-overridable, default `6`) counting every `dev-00-gh-runner-*` Hetzner server in any status. Before emitting `runner_need=true` the script takes a short-TTL create lock (an atomic create of the Hetzner **placement group** `runner-create-lock-<size>` — names are unique per project, `uniqueness_error` = lost the race — written with the same `HCLOUD_TOKEN` the callers already use to create VMs; acquisition epoch in the group's `epoch` label; TTL `RUNNER_LOCK_TTL_SECONDS`, default 60s) to close the check-then-act race between concurrent triggers; a held lock sends the run to the wait queue, a stale lock is deleted and re-acquired, and any lock-machinery failure fails open (create without the lock, with a `::warning::`). The lock is org-wide (one shared Hetzner project) with no extra credentials or GitHub permissions — GitHub-ref lock variants failed on token scope, and SSH-key lock objects were rejected because their creation triggers account email notifications.
...
- Keep unit tests advisory (non-blocking) and sequential after lint. Do not add `continue-on-error` to `unit-test` or `integration-tests` steps — the job must still report red when tests fail. Keep the unit gate backward-compatible by resolving to a no-op success for repos without a unit test plan, and keep reporting that no-op as `⏭️ n/a`, not `✅`.
- Keep `build-image` ungated on lint and unit results (advisory only); it must stay gated on `github.event_name != 'pull_request'`.

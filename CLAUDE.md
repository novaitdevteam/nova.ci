...
- The `apiscan*` job is parameterised: `auth-mode` is `login` (POST username/password, read the token from the response — `novatalks.core`), `db-token` (read the seeded token straight out of the DB with a caller-supplied `SELECT` — the telegram connector), or `db-insert` (write a generated token into the DB with a caller-supplied `INSERT`, `%TOKEN%` substituted in — for whatsapp/signal, whose Dockerfiles `npm prune --omit=dev` the seeder away, leaving migrated-but-empty tables with nothing to `SELECT`). The injected header, its scheme prefix and the token `SELECT`/`INSERT` are per-repo inputs; the acquired token is masked with `::add-mask::` whatever its source; an empty token (login returned none, or the `SELECT` matched no row) is a loud skip (`not-run`), never a scan without auth — `db-insert` cannot produce an empty token (it is generated locally), so its own loud skip is a failed `INSERT` instead, meaning the migration never created the table. It runs `zap-api-scan.py` in **safe mode** (`-S`) by default; a `scan-mode` input (`passive` default, `active`) reaches `scan.sh` as `DAST_API_SCAN_MODE` and, only under `active`, drops `-S` — dropping `-S` turns a passive scan into real `POST`/`PUT`/`DELETE` writes against the seeded API, safe only because the stack is the ephemeral one this action starts and kills, so `active` is a deliberate, never-default exception and an unrecognised mode is a scanner error. `scripts/test-dast-api-scan.sh` asserts `-S` on the default, its absence under `active`, and the mask.
...
- Each connector's auth model is verified against its own code before its `targets.sh` arm is written. Telegram's `db-token`/`api_access_token`/`tokens` shape is **not** assumed for the Sequelize connectors (`whatsapp`, `signal`) or `novatalks.dialer` — the tracked Phase 2, each with its own token storage, header and seed path.

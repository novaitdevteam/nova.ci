...
- The `apiscan*` job is parameterised: `auth-mode` is `login` (POST username/password, read the token from the response — `novatalks.core`) or `db-token` (read the seeded token straight out of the DB with a caller-supplied `SELECT` — the telegram connector). The injected header, its scheme prefix and the token `SELECT` are per-repo inputs; the acquired token is masked with `::add-mask::` whatever its source; an empty token (login returned none, or the `SELECT` matched no row) is a loud skip (`not-run`), never a scan without auth. It runs `zap-api-scan.py` in **safe mode** (`-S`) by default; a `scan-mode` input (`passive` default, `active`) reaches `scan.sh` as `DAST_API_SCAN_MODE` and, only under `active`, drops `-S` — dropping `-S` turns a passive scan into real `POST`/`PUT`/`DELETE` writes against the seeded API, safe only because the stack is the ephemeral one this action starts and kills, so `active` is a deliberate, never-default exception and an unrecognised mode is a scanner error. `scripts/test-dast-api-scan.sh` asserts `-S` on the default, its absence under `active`, and the mask.

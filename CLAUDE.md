...
- The authenticated **API scan** (`api-scan`) runs `zap-api-scan.py` in **safe mode** (`-S`) against the engine's own `/api-docs-json`, on an `apiscan*` tag for `novatalks.core` only. `-S` is mandatory and the harness asserts it — without it the scan writes to the seeded API. The seeded admin's password is generated per run and never stored (`DEFAULT_ADMIN_USER`/`DEFAULT_USER_PASSWORD`); the engine needs `SWAGGER_ENABLE=true` or the spec is empty and the scan is a loud skip. The JWT is masked from the step log with `::add-mask::` — `tee` sends ZAP's replacer echo to the persisted, world-readable log, which deleting the console file cannot undo. The four OpenAPI connectors (`telegram`, `whatsapp`, `signal`, `dialer`) are the tracked expansion of this scan; extending it is per-repo work, each with its own stack and auth.
- The ZAP tally parse, its ANSI-C `$'…\t…'` anchor and the numeric guard live in `.github/actions/dast/dast-common.sh` and are **sourced** by all three ZAP callers (`dast/scan.sh`, `dast-api/scan.sh`, `ci-dast-live-baseline.yaml`), never re-inlined. A plain `'\t'` matches nothing on GNU grep — that is why it is shared, not copied.
- `.github/workflows/ci-dast-live-baseline.yaml` is a `workflow_dispatch` baseline of a live host, validated against a one-host allowlist; a non-allowlisted target fails before any scan. It notifies through `notify/action.yml` from this repository's own secrets and degrades to artifact-only when they are unset.
...
- The live-baseline workflow (`ci-dast-live-baseline.yaml`) validates its `target` input against a one-host allowlist before scanning; adding a host is a deliberate edit to that `case`, never a runtime choice. It documents the SPA-200 caveat in its own report — duplicate header findings are not extra coverage.

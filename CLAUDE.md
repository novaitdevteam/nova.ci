...
- The `apiscan*` job runs `zap-api-scan.py` in **safe mode** (`-S`) always. Dropping `-S` turns a passive scan into real `POST`/`PUT`/`DELETE` writes against the seeded API using the session the script just created. `scripts/test-dast-api-scan.sh` asserts `-S` is passed.
- The seeded admin password is `openssl rand`-generated per run and stored nowhere — passed via `DEFAULT_ADMIN_USER` / `DEFAULT_USER_PASSWORD`, used once to log in. The engine needs `SWAGGER_ENABLE=true` or `/api-docs-json` is empty and the scan is a loud skip (`not-run`), never a green tick over zero operations. Keep it opt-in and `novatalks.core`-only (`apiscan*` tag). The JWT is masked from the step log with `::add-mask::` before first use: `tee` copies ZAP's replacer echo to the persisted, world-readable step log, which deleting the console file cannot undo. The four OpenAPI connectors (`telegram`, `whatsapp`, `signal`, `dialer`) are the tracked expansion; extending it is per-repo work with its own stack and auth.

...
- The lock is a **Hetzner placement group** named `runner-create-lock-<size>` in the same Hetzner project the runner VMs live in. Placement group names are unique per project, so `POST /placement_groups` is atomic: a `uniqueness_error` means somebody else holds the lock. A placement group is free, pure metadata, and creating one triggers no account notifications.
- It is written with the same `HCLOUD_TOKEN` every caller already passes to create runner VMs, so the lock is **org-wide** (one shared Hetzner project) with **no extra credentials and no GitHub permissions**. (GitHub-side variants — a lock ref in a shared repo or in the caller's own repo — all foundered on token scope: the org PAT has no `contents: write` anywhere relevant, and the built-in `GITHUB_TOKEN` would need per-caller wiring.)
- The group's `epoch` label carries the acquisition timestamp. A lock younger than `RUNNER_LOCK_TTL_SECONDS` (env-overridable, default `60`) sends the run to the wait queue with a `::notice::`; an older, far-future, or unreadable lock is treated as stale, deleted, and re-acquired.
- Nobody releases the lock explicitly — it expires via TTL, by which time the winner's VM is visible to the per-size count, which takes over as the guard; the next trigger for that size clears the stale group.
- The lock machinery fails **open**: any API failure emits a `::warning::` and proceeds without the lock (i.e. degrades to the small pre-lock race window) rather than blocking runner creation.
